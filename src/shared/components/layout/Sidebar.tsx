  ...
  return (
    ...
    >
      ...
      <div className="border-t border-white/10 px-2 py-3 space-y-0.5 bg-black/10">
        {canAdministerOrg && !compact && <p className="px-2 pb-1 text-[10px] font-semibold uppercase tracking-widest text-slate-600">Organization setup</p>}
        {canAdministerOrg && BOTTOM_NAV.map((item) => (
          <NavLink
            key={item.key}
            to={item.href}
            onClick={() => setMobileOpen(false)}
